...
(function () {
  ...
  if (!DATA || !JY) { document.body.innerHTML = '<p style="padding:40px;font-size:18px">加载失败：请确认 js/data.js 与 js/engine.js 存在。</p>'; return; }
  ...
  var DIST_NAMES = ['0 贴身', '1 近身', '2 中距', '3 远距', '4 极远'];
  var WEAPON_ICON = { '剑': '⚔', '拳': '👊', '刀': '🗡', '暗': '🎯' };
  var PORTRAIT = { liu: '剑', tie: '拳', li: '刀', tang: '针', zhang: '极' };
  ...
  function portraitHtml(heroId, size) {
    return '<span class="portrait ' + heroId + '" style="width:' + size + 'px;height:' + size + 'px">' +
      '<b>' + (PORTRAIT[heroId] || '侠') + '</b><i>' + (PORTRAIT_ICON[heroId] || '') + '</i></span>';
  }
  ...
  // ================= 工具 =================
  function who(pIdx) { return pIdx === 0 ? '玩家一' : '玩家二'; }
  ...
  function renderStats() {
    ...
    var html = '<div class="st-line">总局数 <b>' + st.games + '</b>　玩家一胜 <b>' + p1w + '</b>　玩家二胜 <b>' + p2w + '</b>　平局 <b>' + dr + '</b></div>';
    if (st.games > 0) {
      html += '<div class="st-line">玩家一胜率 <b>' + Math.round(p1w / st.games * 100) + '%</b>　玩家二胜率 <b>' +
        Math.round(p2w / st.games * 100) + '%</b>　平均回合 <b>' + Math.round(st.turnsSum / st.games) + '</b></div>';
    }
    var heroes = Object.keys(st.heroes).sort(function (a, b) { return (st.heroes[b] || 0) - (st.heroes[a] || 0); });
    if (heroes.length) {
      html += '<div class="st-line">侠客胜场：' + heroes.slice(0, 6).map(function (h) {
        return '<b>' + h + '</b>×' + st.heroes[h];
      }).join('　') + '</div>';
    }
    // 对阵胜率矩阵
    var MATRIX_ORDER = ['柳如风', '铁无双', '厉斩风', '唐十七', '张玄清'];
    var hasPairs = Object.keys(st.pairs).length > 0;
    if (hasPairs) {
      html += '<div class="st-line">对阵胜率（行胜列）：</div><table class="matrix"><tr><th></th>';
    ...
    html += '<button class="btn small" id="btn-stats-reset">清空战绩</button>';
    box.innerHTML = html;
    var rb = $('btn-stats-reset');
    if (rb) rb.onclick = function () { saveStats(defaultStats()); renderStats(); toast('战绩已清空'); };
  }
  ...
  function spawnStatusSeals() {
    ...
    [0, 1].forEach(function (idx) {
      ...
      if (!snap) return;
      var defs = [
        { d: p.poise - snap.poise, ch: '破', cls: 'poise' },
        { d: p.inner - snap.inner, ch: '伤', cls: 'inner' },
        { d: p.poison - snap.poison, ch: '毒', cls: 'poison' },
        { d: (p.pendingTaiji ? 1 : 0) - (snap.taiji ? 1 : 0), ch: '极', cls: 'taiji' },
        { d: (p.qisurge ? 1 : 0) - (snap.surge ? 1 : 0), ch: '涌', cls: 'qisurge' }
      ];
      var offset = -70;
  ...
  function handleError(e) {
    ...
    showModal('⚠ 出现异常', '错误信息：<b>' + escapeHtml(msg) + '</b><br>请截图或记录此信息，并刷新页面重新开始。',
      [{ label: '知道了', id: 'ok' }], function () { });
  }
  ...
  function toggleSfx() {
    if (SFX) SFX.toggle();
    var btn = $('btn-sfx');
    if (btn) btn.textContent = (SFX && SFX.muted) ? '音效关' : '音效开';
    toast(SFX && SFX.muted ? '音效已关闭' : '音效已开启');
  }
  ...
  function renderSetup() {
    ...
    // 人机模式提示
    var start = $('btn-start');
    start.textContent = UI.p2AI ? '开始对决（人机）' : '开始对决';
  }
  ...
  function renderSceneList() {
    ...
    r.innerHTML = '<div class="sc-name sc-random">🎲 随机场景</div><div class="sc-desc">开局时随机抽取一张场景牌</div>';
  ...
  function renderDraft() {
    ...
    var picker = g.draftTurn === 0 ? '玩家一（' + g.p1.heroName + '）' : '玩家二（' + g.p2.heroName + '）';
    $('draft-status').textContent = '轮到 ' + picker + ' 选择（第 ' + (g.draftCount + 1) + ' 手 / 共 4 手）';
    $('draft-picks').innerHTML =
      '玩家一已选：' + (g.p1.hand.length - 8) + ' 张　|　玩家二已选：' + (g.p2.hand.length - 8) + ' 张';
  }
  ...
  function renderYuanhu() {
    ...
    h.textContent = who(pIdx) + '（' + p.heroName + '）密邀援护';
    var note = document.createElement('div');
    note.className = 'pass-notice';
    note.textContent = '请对手移开视线：从以下 3 位江湖奇人中暗选 1 位（发动前对对手保密）';
    stage.appendChild(h);
  ...
  function beginSelectPhase() {
    ...
    if (p.forcedCard) {
      ...
      toast(who(UI.selector) + ' 被摄魂大法控制，本回合必须打出「' + p.forcedCard.name + '」');
    }
    renderAll();
  }
  ...
  function onCardClick(pIdx, card) {
    ...
    if (cost > p.qi) {
      ...
      if (hasAffordable) { toast('真气不足，无法出此招'); return; }
      toast('真气不足：出招将视为落空');
    }
    UI.sel[pIdx] = card;
    sfx('select');
  ...
  function renderHeader() {
    var g = UI.game;
    $('g-turn').textContent = '第 ' + (g.turn + 1) + ' 回合';
    $('g-scene').textContent = g.scene ? '场景：' + DATA.SCENES[g.scene].name : '无场景 · 华山之巅';
    var map = {
      adjust: '调息阶段', select: '出招阶段', pass: '交接设备', reveal: '亮牌对决', roundEnd: '回合结束'
    };
  ...
  function revealCardHtml(card) {
    ...
    rc += '<div class="rc-name">' + escapeHtml(card.name) +
      (card.ultimate ? '<em class="rc-seal">绝学</em>' : '') +
      (card.isPublic ? '<em class="rc-seal pub">秘</em>' : '') + '</div>';
    rc += '<div class="rc-meta">速' + card.speed + ' · 耗' + card.cost + ' · ' +
      DATA.rangeText(card.lo, card.hi) + ' · 伤' + card.dmg + '</div>';
  ...
  function handCardHtml(pIdx, card) {
    ...
    return '<div class="' + cls + '" data-uid="' + card.uid + '">' +
      (card.ultimate ? '<span class="c-ult">绝学</span>' : '') +
      (card.isPublic ? '<span class="c-ult" style="background:var(--green)">秘</span>' : '') +
      '<div class="c-name">' + escapeHtml(card.name) + '</div>' +
      '<div class="c-meta"><span>速<b>' + card.speed + '</b></span><span>伤<b>' + card.dmg + '</b></span></div>' +
      '<div class="c-range">' + DATA.rangeText(card.lo, card.hi) + '</div>' +
      '<div class="c-cost">耗<b>' + costTxt + '</b></div>' +
  ...
  function renderPanel(pIdx) {
    ...
    var html = '<div class="pp-head">' +
      ...
      '<span class="pp-tag">' + (WEAPON_ICON[p.title === '剑侠' ? '剑' : (p.title === '拳师' || p.title === '太极') ? '拳' : (p.title === '刀狂' ? '刀' : '暗')] || '') + ' ' +
      (g.dlc4 ? (p.yuanhuUsed ? '<span class="badge used">援护已发</span>' : '<span class="badge yuanhu">援护：？</span>') : '') + '</span></div>';

    var hpPct = Math.max(0, Math.min(100, Math.round(p.hp / p.maxhp * 100)));
    html += '<div class="hp-row"><span class="hp-label">气血</span>' +
    ...
    html += '<div class="qi-row"><span class="hp-label">真气</span><div class="qi-pips">' + qipips + '</div></div>';

    var badges = '';
    function badge(cls, txt) { return '<span class="badge ' + cls + '">' + txt + '</span>'; }
    if (p.poise > 0) badges += badge('poise', '破绽×' + p.poise);
    if (p.inner > 0) badges += badge('inner', '内伤×' + p.inner);
    if (p.poison > 0) badges += badge('poison', '中毒×' + p.poison);
    if (p.pendingTaiji) badges += badge('taiji', '太极势');
    if (p.slowActiveVal > 0) badges += badge('slow', '减速' + p.slowActiveVal);
    if (p.qisurge) badges += badge('qisurge', '气涌');
    if (p.immuneRound) badges += badge('immune', '免疫');
    if (p.shields.length) badges += badge('shield', '护盾×' + p.shields.length);
    html += '<div class="status-row">' + (badges || '<span style="font-size:12px;color:var(--ink-4)">无状态</span>') + '</div>';

    html += '<div class="pp-sub"><span>回气堆 <b>' + p.discard.length + '</b>/' + g.discardLimit + '</span>' +
      '<span>手牌 <b>' + p.hand.length + '</b></span>' +
      '<span>绝学 <b>' + (p.ultUsed ? '已用' : '可用') + '</b></span></div>';
    ...
    function flipWrap(faceUp, cardHtml) {
      ...
      return '<div class="flip-wrap"><div class="flip-inner' + innerCls + '">' +
        '<div class="flip-face flip-front"><span class="flip-mark">剑</span></div>' +
    ...
    if (UI.phase === 'reveal') {
      ...
      token = '<div class="play-token' + (faceUp ? ' revealed' : '') + '">' +
        ...
        '<span class="token-note">' + (faceUp ? '已亮牌' : '盖牌待翻') + '</span>' +
      ...
      if (cur) {
        token = '<div class="play-token revealed">' +
          ...
          '<div class="token-side">' +
          '<span class="token-note">' + (cur.ultimate ? '绝学 · 速' + cur.speed : '速' + cur.speed + ' · 伤' + cur.dmg) + '</span>' +
      ...
      if (forced) {
        token = '<div class="play-token revealed">' +
          ...
          '<div class="token-side">' +
          '<span class="token-note">被摄魂大法控制，必须打出此牌</span>' +
          '<span class="token-effect">' + escapeHtml(selected.desc || '') + '</span>' +
          '</div></div>';
      } else {
        token = '<div class="play-token">' +
          flipWrap(false, '') +
          '<div class="token-side"><span class="token-note">已选招式（盖牌）</span></div></div>';
      }
    }
    html += token;

    // 手牌
    html += '<div class="hand-label">手牌（' + (handVisible ? '选择中' : '盖牌') + '）</div><div class="hand">';
    if (handVisible) {
      p.hand.forEach(function (card) { html += handCardHtml(pIdx, card); });
      if (!p.ultUsed) html += handCardHtml(pIdx, p.ult);
    } else {
      for (var b = 0; b < p.hand.length; b++) html += '<div class="card back"><div class="back-mark">剑</div></div>';
    }
    html += '</div>';
    el.innerHTML = html;
  ...
  function handleCombatFx(e) {
    ...
    r = /反弹\s+(\d+)\s*点\s*给\s*([^\s]+)/.exec(t) || /将\s+(\d+)\s*点伤害.*?反弹给\s+([^\s]+)/.exec(t);
    if (r) {
  ...
  function renderActionBar() {
    ...
    switch (UI.phase) {
      case 'adjust':
        hint('调息阶段…', '回复真气、移除破绽、中毒结算');
        break;
      case 'select': {
        var p = UI.game.players[UI.selector];
        var forced = !!p.forcedCard;
        if (isAI(UI.selector)) {
          hint('电脑（' + p.heroName + '）思考中…', forced ? '被摄魂大法控制，必须打出「' + p.forcedCard.name + '」' : '正在权衡出招');
        } else {
          hint('轮到 ' + who(UI.selector) + '（' + p.heroName + '）选择招式',
            forced ? '被摄魂大法控制：本回合必须打出「' + p.forcedCard.name + '」' : '点击手牌选中，再点「确认出招」');
          if (UI.sel[UI.selector]) {
            addBtn('确认出招', 'btn-primary', function () { confirmSelection(UI.selector); });
            if (!forced) addBtn('重选', '', function () { UI.sel[UI.selector] = null; renderAll(); });
          }
        }
        break;
      }
      case 'pass': {
        ...
        hint('请将设备交给 ' + who(next), '不要让对手看到你的选择');
        addBtn('我已拿到设备，开始选牌', 'btn-primary', function () {
          UI.selector = next;
          UI.phase = 'select';
          beginSelectPhase();
        });
      ...
      case 'reveal':
        if (!UI.flipDone) {
          ...
          hint('亮牌！', '查看双方所选招式后开始结算');
          addBtn('⚔ 开始结算', 'btn-primary', function () { reveal(); });
        }
        break;
      case 'roundEnd':
        hint('本回合结束', '');
        addBtn('进入下一回合', 'btn-primary', function () { nextRound(); });
        break;
    }
  }
  ...
  function buildBigCard(card, extraCls) {
    ...
    div.innerHTML =
      (card.ultimate ? '<span class="c-ult">绝学</span>' : '') +
      '<div class="c-name">' + escapeHtml(card.name) + '</div>' +
      '<div class="c-meta"><span>速<b>' + card.speed + '</b></span><span>耗<b>' + card.cost + '</b></span><span>伤<b>' + card.dmg + '</b></span></div>' +
      '<div class="c-range">' + DATA.rangeText(card.lo, card.hi) + '</div>' +
      '<div class="c-desc">' + escapeHtml(card.desc || '') + '</div>';
    return div;
  }
  ...
  function showCodex() {
    ...
    var html = '';
    // 规则速查
    html += codexSection('规则速查', '<div class="codex-rules">' +
      '<p><b>目标：</b>将对手气血打到 0 或以下；双方同时为 0 判平；60 回合未分胜负判平。</p>' +
      '<p><b>距离：</b>共享距离轨 0 贴身 ~ 4 极远，初始 2。招式不在适用距离内则落空（+1 破绽）。</p>' +
      '<p><b>真气：</b>上限 5，起始 5，每回合开始回复 1；出招须消耗牌面真气。</p>' +
      '<p><b>破绽：</b>每层使出招速度 -1，回合开始自动移除 1 层。<b>内伤：</b>每层使受到的伤害 +1。<b>中毒：</b>回合开始受等量伤害后衰减 1 层。</p>' +
      '<p><b>回气：</b>打出的招式进入回气堆，达 4 张（DLC2 为 5）全部返回手牌。</p>' +
      '<p><b>对决：</b>同时亮牌比速度（招式速度 - 破绽 + 太极势 - 减速），高者先手、低者后手；平速双方视为后手且伤害 -1（最低 1）。</p>' +
      '<p><b>绝学：</b>一局一次，使用后移出游戏；落空同样移出并 +1 破绽。</p>' +
      '<p><b>气涌：</b>下次出招耗气 -1。<b>太极势：</b>下回合出招速度 +2。<b>减速：</b>下回合出招速度 -N。</p>' +
      '</div>');
    // 侠客
    HERO_ORDER.forEach(function (hid) {
      var h = DATA.CHARACTERS[hid];
      var cards = h.cards.slice();
      html += codexSection(hid === 'liu' ? '剑侠 · 柳如风' : hid === 'tie' ? '拳师 · 铁无双' : hid === 'li' ? '刀狂 · 厉斩风' : hid === 'tang' ? '千手 · 唐十七' : '太极 · 张玄清',
        '<p class="codex-style">' + escapeHtml(h.style) + '</p>' +
        codexCards(cards) +
        '<p class="codex-ult-label">绝学</p>' + codexCards([h.ult]));
    });
    // 公共牌
    html += codexSection('武林秘藏 · 公共招式（DLC2）', codexCards(DATA.PUBLIC_CARDS));
    // 场景
    var sc = Object.keys(DATA.SCENES).map(function (k) {
      var x = DATA.SCENES[k];
      return '<div class="codex-item scene"><b>' + x.name + '</b>　' + escapeHtml(x.desc) + '</div>';
    }).join('');
    html += codexSection('十方奇境 · 场景（DLC3）', sc);
    // 援护
    var yh = DATA.YUANHU_CARDS.map(function (y) {
      return '<div class="codex-item"><b>' + y.name + '</b>　' + escapeHtml(y.desc) + '</div>';
    }).join('');
    html += codexSection('双侠并起 · 援护（DLC4）', yh);
    body.innerHTML = html;
  }
  ...
  function showGameOver() {
    var g = UI.game;
    var title, cls, sub;
    if (g.winner === 'p1') { title = g.p1.heroName + ' 获胜'; cls = 'p1'; sub = '玩家一（' + g.p1.heroName + '）执掌武林牛耳！'; }
    else if (g.winner === 'p2') { title = g.p2.heroName + ' 获胜'; cls = 'p2'; sub = '玩家二（' + g.p2.heroName + '）执掌武林牛耳！'; }
    else if (g.winner === 'draw') { title = '平局'; cls = 'draw'; sub = '双方同归于尽，正邪并亡，传为武林轶事。'; }
    else { title = '平局'; cls = 'draw'; sub = '六十回合已过，胜负未分，江湖再会。'; }
    // 战绩记录 + 音效 + 墨爆 + 绝杀定格
    ...
    var html = '<div class="win-banner">' +
      ...
      '<div class="win-sub">历时 ' + g.turn + ' 回合　|　' +
      '玩家一气血 ' + Math.max(0, g.p1.hp) + ' / ' + g.p1.maxhp + '　|　' +
      '玩家二气血 ' + Math.max(0, g.p2.hp) + ' / ' + g.p2.maxhp + '</div></div>';
    showModal('对决结束', html, [
      { label: '再来一局（同配置）', id: 'again' },
      { label: '返回设置', id: 'setup' }
    ], function (id) {
  ...
  function bindEvents() {
    ...
    $('btn-quit').onclick = function () {
      showModal('退出对局', '确定要结束本局并返回设置界面吗？', [
        { label: '确定退出', id: 'yes' },
        { label: '继续对战', id: 'no' }
    ...
    $('opt-dark').onchange = function () {
      ...
      applyTheme(dark);
      saveTheme(dark);
      toast(dark ? '已切换夜墨深色主题' : '已切换宣纸浅色主题');
    };
    // 快捷键
    bindKeyboard();
    // 音效开关
    ...
    function updateSfxBtn() {
      ...
      btnSfx.textContent = muted ? '音效关' : '音效开';
    }
    if (btnSfx) {
